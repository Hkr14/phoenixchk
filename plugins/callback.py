...
async def myacc(Client, message , update):
  ...
  if isinstance(find, type(None)) == True:
    ...
    text = f"""
 User Information:-
 First Name: <b>{message.reply_to_message.from_user.first_name}</b>
 User Name: <b>{message.reply_to_message.from_user.username}</b>
 User Id: <b><code>{message.reply_to_message.from_user.id}</code></b>
 Limited: <b>{message.reply_to_message.from_user.is_restricted}</b>
 Profile Link: <b><a href="tg://user?id={message.reply_to_message.from_user.id}">Click Here</a></b>
 Profile Image: <b><a href="{find['image']}">Click Here</a></b>

 User Database Information:-
 Role: <b>{find['role']}</b>
 Plan: <b>{find['plan']}</b>
 Status: <b>{find['status']}</b>
 Credits: <b>{find['credits']}</b>
 Live Cards: <b>COMING SOON</b>
 AntiSpam Time: <b>{datetime.utcfromtimestamp(antispam_time).strftime('%H:%M:%S %d-%m-%Y')}</b>

 Chat Information:-
 Chat Name: <b>{message.reply_to_message.chat.title}</b>
User Name: <b>{message.reply_to_message.chat.username}</b>
 Chat Id: <b><code>{message.reply_to_message.chat.id}</code></b>
 Chat Type: <b>{message.reply_to_message.chat.type.capitalize()}</b>
      """
...
async def buy(Client, message , update):
  buttons = [
    [
        InlineKeyboardButton('🛒 BUY 🛒', url='https://t.me/Sarcehkr'),
        InlineKeyboardButton('ℹ CHANNEL ℹ', url='https://t.me/PhoenixCh')
    ],
    [
        InlineKeyboardButton('↩️ RETURN ↩️', callback_data='gates'),
        InlineKeyboardButton('🚪 CLOSE 🚪', callback_data='close')
